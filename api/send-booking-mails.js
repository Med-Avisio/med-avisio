...
if (patientMail.error) {
  console.error('Patient mail error:', patientMail.error);
  return res.status(500).json({ error: patientMail.error.message || patientMail.error });
}

const adminMail = await resend.emails.send({
  from: fromAddress,
  to: 'daniela@erhard-mail.de',
...
if (adminMail.error) {
  console.error('Admin mail error:', adminMail.error);
  return res.status(500).json({ error: adminMail.error.message || adminMail.error });
}
